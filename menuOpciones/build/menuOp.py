
# This file was generated by the Tkinter Designer by Parth Jadhav
# https://github.com/ParthJadhav/Tkinter-Designer


from pathlib import Path

# from tkinter import *
# Explicit imports to satisfy Flake8
from tkinter import Tk, Canvas, Entry, Text, Button, PhotoImage, Label


OUTPUT_PATH = Path(__file__).parent
ASSETS_PATH = OUTPUT_PATH / Path(r"/Users/jezzzcan/Desktop/TT2_Brainboard/Tkinter-Designer-master/menuOpciones/build/assets/frame0")


def relative_to_assets(path: str) -> Path:
    return ASSETS_PATH / Path(path)

def on_enter(label, hover_image):
    label.config(image=hover_image)

def on_leave(label, normal_image):
    label.config(image=normal_image)

def on_click(label, pressed_image):
    label.config(image=pressed_image)

def create_label(x, y, normal, hover, pressed, command):
    normal_image = PhotoImage(file=relative_to_assets(normal))
    hover_image = PhotoImage(file=relative_to_assets(hover))
    pressed_image = PhotoImage(file=relative_to_assets(pressed))
    
    label = Label(window, image=normal_image, borderwidth=0, highlightthickness=0, bg="#FFFFFF")
    label.image = normal_image
    label.place(x=x, y=y)
    
    label.bind("<Enter>", lambda event: on_enter(label, hover_image))
    label.bind("<Leave>", lambda event: on_leave(label, normal_image))
    label.bind("<ButtonPress-1>", lambda event: on_click(label, pressed_image))
    label.bind("<ButtonRelease-1>", lambda event: command())
    
    return label

window = Tk()

window.geometry("1280x719")
window.configure(bg = "#FFFFFF")

window.update_idletasks()
screen_width = window.winfo_screenwidth()
screen_height = window.winfo_screenheight()
window_width = 1280
window_height = 720
x_position = (screen_width - window_width) // 2
y_position = (screen_height - window_height) // 2
window.geometry(f"{window_width}x{window_height}+{x_position}+{y_position}")


canvas = Canvas(
    window,
    bg = "#FFFFFF",
    height = 719,
    width = 1280,
    bd = 0,
    highlightthickness = 0,
    relief = "ridge"
)

canvas.place(x = 0, y = 0)
image_image_1 = PhotoImage(
    file=relative_to_assets("image_1.png"))
image_1 = canvas.create_image(
    640.0,
    359.0,
    image=image_image_1
)

canvas.create_text(
    332.0,
    83.0,
    anchor="nw",
    text="¡Bienvenido a BrainBoard!",
    fill="#FFFFFF",
    font=("Inter ExtraBold", 48 * -1)
)

canvas.create_text(
    396.0,
    147.0, #y (horizontal)
    anchor="nw",
    text="Selecciona una opción:",
    fill="#FFFFFF",
    font=("Inter Black", 40 * -1)
)

image_image_2 = PhotoImage(
    file=relative_to_assets("image_2.png"))
image_2 = canvas.create_image(
    84.0,
    61.0,
    image=image_image_2
)

# Crear labels en lugar de botones
create_label(981.0, 559.0, "button_1.png", "button_1_hover.png", "button_1_pressed.png", lambda: print("button_1 clicked"))
create_label(421.0, 551.0, "button_2.png", "button_2_hover.png", "button_2_pressed.png", lambda: print("button_2 clicked"))
create_label(416.0, 438.0, "button_3.png", "button_3_hover.png", "button_3_pressed.png", lambda: print("button_3 clicked"))
create_label(412.0, 327.0, "button_4.png", "button_4_hover.png", "button_4_pressed.png", lambda: print("button_4 clicked"))
create_label(414.0, 215.0, "button_5.png", "button_5_hover.png", "button_5_pressed.png", lambda: print("button_5 clicked"))

window.resizable(False, False)
window.mainloop()
